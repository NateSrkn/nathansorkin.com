...
export const AnimatedText = ({children, type, ...other}) => {
  ...
  const wrapText = () => {
    ...
    for (let text of textWrapper) {
      if (text.parentElement.classList.value.includes('subheader') && currentPage.includes('project')) return 
  ...
  const animateText = () => {
    ...
    if (currentPage.includes('project')) {
      ...
      const subheaders = document.querySelectorAll('.text-wrapper.subheader .letter')
